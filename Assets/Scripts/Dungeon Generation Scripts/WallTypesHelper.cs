/*
 * This file contains different hash sets of binary numbers.
 * These are used to help generate the correct wall types.
 * This is done by iterating through our wall position sets and assigning 1s or 0s 
 * to a string value in WallGenerator.cs (neighborBinaryType)
 * depending on the neighbors of the selected position
 * if the neighbor is a floor, we add a 1
 * if the nighbor is not a floor, we add a 0
 * a four(basic) or eight(corner) bit number is generated 
 * and is compared to one of the binary numbers here
 * to determine which type of wall should be generated
 */
